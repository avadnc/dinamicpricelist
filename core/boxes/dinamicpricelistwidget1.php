...
/**
 * \file    dinamicpricelist/core/boxes/dinamicpricelistwidget1.php
 * \ingroup dinamicpricelist
 * \brief   Widget provided by Dinamicpricelist
 *
 ...
 */
class dinamicpricelistwidget1 extends ModeleBoxes
{
    /**
     * @var string Alphanumeric ID. Populated by the constructor.
     */
    public $boxcode = "dinamicpricelistbox";

    /**
     * @var string Box icon (in configuration page)
     * Automatically calls the icon named with the corresponding "object_" prefix
     */
    public $boximg = "dinamicpricelist@dinamicpricelist";
     ...
     */
    public $depends = array('dinamicpricelist');
    ...
    public function __construct(DoliDB $db, $param = '')
    {
        ...
        //$this->hidden = ! ($user->rights->dinamicpricelist->myobject->read);   // Condition when module is visible by user (test on permission)
    }
    ...
    public function loadBox($max = 5)
    {
        ...
        //dol_include_once("/dinamicpricelist/class/dinamicpricelist.class.php");

        // Populate the head at runtime
        $text = $langs->trans("DinamicpricelistBoxDescription", $max);
        $this->info_box_head = array(
            ...
            // 'subpicto' => 'object_dinamicpricelist@dinamicpricelist',
            // Sublink icon HTML alt text
            // 'subtext' => 'Esto es subtexto',
            // Sublink HTML target
            // 'target' => 'products.php',
        ...
        if ($num > 0) {
            ...
            while ($i < $num) {
                ...
                if ($obj->code != $conf->currency) {
                    ...
                    // $result_array = [
                    //     'currency' => $obj->code,
                    //     'rate' => $currency->rates[$i]->rate,
                    //     'date' => $currency->rates[$i]->date_sync,
                    // ];

                    // $i => array( // Another line
                    //     0 => array( // TR
                    //         'tr' => 'class="left"',
                    //         'text' => $obj->code,
                    //     ),
                    //     1 => array( // TR
                    //         'tr' => 'class="left"',
                    //         'text' => $currency->rates[$i]->rate . ' - ' . dol_print_date($currency->rates[$i]->date_sync),
                    //     ),
                    // ),

                    for ($j = 0; $j < 5; $j++) {
                       
                        $boxes_arr = array( // First line
                            ...
                            1 => array( // Another column
                                ...
                                'textnoformat' =>  '$'.$currency->rates[$j]->rate . ' MXN - ' . dol_print_date($currency->rates[$j]->date_sync),
								 'asis' => false,
                            ),
                        );
                        array_push($boxes, $boxes_arr);
                    }
                }
